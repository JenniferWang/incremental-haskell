...
\paragraph{} It turns out that there are already some existing libraries in Haskell related to incremental computing: \textit{Adaptive}\cite{Adaptive} is one directly related to Incremental Computing, but the library was based on some theoretical results int the year 2002 and the theory has evolved since then. There are also many Functional Reactive Programming libraries in Haskell, such as \textit{Reactive-banana}\cite{reactive}. However, this library is mostly focused on GUI applications rather than computations.
...
\paragraph{} Representing a dynamic graph in Haskell is not as straightforward as tree. For efficiency consideration, we decided to give up purity and use \code{IORef}. The good news is that we only need to keep one copy of each node, while on the other hand, most of the manipulation will involve \code{IO} monad.

\subsection{Node}
\paragraph{} \code{Node a} represents a node in the DAG. Each node needs to maintain a list of fields which might be updated during the stabilization. To avoid copying large record, we organize the fields in a hierarchical structure and use \code{Lens.Simple} to get easy access and mutation.
...
\paragraph{} This example also shows that it is user's responsibility to make sure that the function fed into \code{Bind} node have no other side effect except for introducing new nodes or observers. It is easy to crash the system with inappropriate \code{IO} actions and thus we are trying to restrict misuse on the type level to improve safety.

\subsection{Exception Handling}
\paragraph{} Runtime exception is inevitable as incremental provides users with the flexibility to construct the DAG on the fly. However, the exception handling fuctions in \code{Control.Exception.Base} only work with \code{IO} monad, not monad transformers. Exception handling in monad transformers could be tricky (see \cite{exception}). Luckily, \code{exceptions} package gives a clean solution in our case.
...
\paragraph{} Our current algorithm keeps a global state to keep track of the graph and uses a DFS-based topological sorting algorithm to do recomputation. A tempting future work is to make the recomputation parallel, which is not possible using current algorithm.
